...
The name of each question is important. The name will be the name of the code chunk header with the exercise/question followed by a number starting at 1. In the following questions you will see that I named the code chunks "A", "B", "C", ... for simplicity. If you want different questions to be worth more or less points, you can specify that in the `points_possible` vector.
...
Then we can output the tutorial header. I recomment putting this before any sections so that it will always be visible no matter what part of the tutorial the student is on. The `grade_server()`, `grade_button_ui`, and `grade_output_ui` should all have the same `id`.
...
-   Submit the completed pdf to a learning management system like Canvas or Gradescope.
...
To insert a question with rearrange matching into a `learnr` quiz, use `question_matching()`.

`choices` should be a vector of statements that will remain stationary in the left column.
