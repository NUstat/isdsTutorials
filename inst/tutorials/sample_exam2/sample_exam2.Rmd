...
Build a multiple linear model that predicts `calories` using `total_fat` and `carbohydrates`. Store the model and print the `summary()`.
...
Build a multiple regression model with an **interaction** term that predicts `calories` based on `total_fat` and `category`. The variable `category` has 2 levels: {food, drink}.
...
Use `model_calories` to add the `residuals()` to the `mcdonalds` dataset. Hint: remove NA values from the dataset and then add the residuals. Print/output the results.
