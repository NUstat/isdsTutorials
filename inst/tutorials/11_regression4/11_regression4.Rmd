...
How many quantities does this model estimate? That is, how many values will there be in the Estimate column of a regression table for this model? ", 
   ...
   # Q7
   question_wordbank("Q7) Match each model code with the appropriate mathematical representation.",
          choices = c("lm(debt ~ credit_limit + income)",
                      "lm(score ~ age + gender + age*gender)"),
          wordbank = c("\\(\\hat{y} = b_1*x_1 + b_2*x_2 \\)", "\\(\\hat{y} = b_1*x_1 + b_2*x_2 + b_3*x_1*x_2 \\)", 
                       "\\(\\hat{y} = b_0 + b_1*x_1 + b_2*x_2 + b_3*x_1*x_2 \\)", "\\(\\hat{y} = b_0 + b_1*x_1 + b_2*x_2 \\)"),
           answer(c("\\(\\hat{y} = b_0 + b_1*x_1 + b_2*x_2 \\)",
                    "\\(\\hat{y} = b_0 + b_1*x_1 + b_2*x_2 + b_3*x_1*x_2 \\)"), correct = TRUE),
           allow_retry = TRUE),
   # Question 17
   question("Match the model with the appropriate mathematical representation:\n
          ",
           ...
           allow_retry = TRUE),
  # Question 18
  question("After fitting score_model_interaction to the evals_ch6 dataset, what is the value of  $y - \\hat{y}$ for the 4th instructor? Round to 3 decimal places.", 
           type = "learnr_text",
           answer("0.548", correct = TRUE),
           allow_retry = TRUE),
  # Question 19
  question("Suppose you ran the following lines of code:\n
